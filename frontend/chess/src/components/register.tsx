...
const Register = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      // Await the response from the registerUser function
      const response = await registerUser(data.email, data.username, data.password);
  
      // Assuming the response is a JSON object, you can directly process it
      ...
      // Catch any errors and set them manually in the form
      setError("root", {
        type: "manual",
        message: err instanceof Response ? (await err.json()).detail : "An error occurred",
      });
    }
  };
